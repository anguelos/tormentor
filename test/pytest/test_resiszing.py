...
def test_padto(pad_size):
    ...
    if plot_dbg:
        ...
        ax[0][1].imshow(in_img_sf[0], vmin=-1., vmax=1.);
        ax[1][1].imshow(in_img_sf[1], vmin=-1., vmax=1.)
        print("in_img_sf", in_img_sf[0].mean())

        ax[0][2].imshow(new_img_sf[0], vmin=-1., vmax=1.);
        ax[1][2].imshow(new_img_sf[1], vmin=-1., vmax=1.)
...
@pytest.mark.parametrize("fixed_size", [(100, 100), (200, 100), (200, 100), (200, 200), (128, 128)])
def test_padcropto(fixed_size):
    width, height = fixed_size
    aug = tormentor.PadCropTo.new_size(*fixed_size)()
    new_img = aug(in_img)
    assert new_img.size(1) == height and new_img.size(2) == width
