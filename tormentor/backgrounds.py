...
class PlasmaBackground(AbstractBackground):
    ...
    def forward_batch_img(self, tensor_image):
        ...
        pixel_means = type(self).pixel_means(batch_size).view(-1, 1, 1, 1).to(tensor_image.device)
        pixel_ranges = type(self).pixel_ranges(batch_size).view(-1, 1, 1, 1).to(tensor_image.device)
